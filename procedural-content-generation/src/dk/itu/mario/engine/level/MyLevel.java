...
public class MyLevel extends Level{
	...
	public void create(MyDNA dna, int type)
	{
        ...
        char [] chromeArr = chromosome.toCharArray();
        int [] gaps = new int [chromeArr.length];
		int randnum1 = random.nextInt(2);
        int randnum2 = random.nextInt(15);
		int count1=0;
		int count2=0;

        for (int i = STARTOFFSET; i < width-EXITOFFSET; i++) {
            if(count1 == randnum1 && count2!=randnum2){
                randnum2 = random.nextInt(15);
                randnum1 = random.nextInt(3);
                count2++;
                if(count2==randnum2)
                    count1 = 0;
                if(i<gaps.length)
		...
		for(int i=0; i<chromeArr.length; i++)
        {
            ...
            if(a=='x' && gaps[i]==1){
                p=1;
                this.setBlock(i + STARTOFFSET, DEFAULTHEIGHT-3-p, ROCK);
                this.setBlock(i + STARTOFFSET, DEFAULTHEIGHT-3, ROCK);
            }
            else if(a=='y' && gaps[i]==1){
                randnum1 = random.nextInt(10);
                if(randnum1<3)
                    decorate(i,i+5, DEFAULTHEIGHT-3-p, 10);
                else {
                    this.setBlock(i + STARTOFFSET, DEFAULTHEIGHT - 3-p, COIN);
                    this.setBlock(i + STARTOFFSET, DEFAULTHEIGHT - 4-p, COIN);
                }
            }
            else if(a=='z' && gaps[i]==1){
                randnum1 = random.nextInt(10);
                if(randnum1<3)
                    addEnemyLine(i,i+6, DEFAULTHEIGHT-3-p, 18);
                else
                    this.setSpriteTemplate(i + STARTOFFSET, DEFAULTHEIGHT-3-p, new SpriteTemplate(2, false));
            }
            else if(a=='d' && gaps[i]==1 && i!=0)
            {
                if(gaps[i-1]==1)
                {
                    char b=chromeArr[i-1];
                    if((b=='x' || b=='d') && this.BLOCKS_POWER<2){
                        this.setBlock(i + STARTOFFSET, DEFAULTHEIGHT - 6-p, BLOCK_POWERUP);
                        this.BLOCKS_POWER = this.BLOCKS_POWER+1;
                        this.setBlock(i + STARTOFFSET, DEFAULTHEIGHT-3, ROCK);
                    }
                    else if(b=='y' || b=='z'){
                        this.setBlock(i + STARTOFFSET, DEFAULTHEIGHT - 4-p, COIN);
                        this.setBlock(i + STARTOFFSET, DEFAULTHEIGHT - 5-p, BLOCK_COIN);
                        this.BLOCKS_COINS=this.BLOCKS_COINS+1;
                        this.setBlock(i + STARTOFFSET, DEFAULTHEIGHT - 3-p, COIN);
                    }
                }
            }
