...
public class MyLevelGenerator{
	...
	private MyDNA generateRandomIndividual ()
	{
		...
		char[] letters = {'x','y','z','d'};
		char[] chromosome = new char[450];
		int i =0;
		Random rand = new Random();
		while(i<450){
			int rand_int = rand.nextInt(letters.length);
			char letter = letters[rand_int];
			chromosome[i] = letter;
			i++;
		}
		individual.setChromosome(new String(chromosome));
	...
	private ArrayList<MyDNA> selectIndividualsForMutation (ArrayList<MyDNA> population)
	{
		...
		// YOUR CODE GOES BELOW HERE
		Random rand = new Random();
		int rand_int1 = rand.nextInt(population.size());
		if(rand_int1>0)
			rand_int1=rand_int1-1;
		int rand_int2 = rand.nextInt(population.size());
		while(rand_int1>=rand_int2){
			rand_int2 = rand.nextInt(population.size());
		}
	...
	private MyDNA pickIndividualForCrossover (ArrayList<MyDNA> population, MyDNA excludeMe)
	{
		...
		// YOUR CODE GOES BELOW HERE
		Random rand = new Random();
		int notToChoose = population.indexOf(excludeMe);
		int index = rand.nextInt(population.size());

		while (notToChoose == index){
	...
	private ArrayList<MyDNA> competeWithParents (ArrayList<MyDNA> oldPopulation, ArrayList<MyDNA> newPopulation, Hashtable parents)
	{
		...
		// YOUR CODE GOES BELOW HERE
		for(MyDNA child: newPopulation){
			ArrayList<MyDNA> temp = new ArrayList<MyDNA>();
			temp.add(child);
			temp.addAll ((ArrayList<MyDNA>) parents.get(child));
			MyDNA best = getBestIndividual(temp);

			if(child.compareTo(best) == 0){
				finalPopulation.add(child);
				newPopulation.remove(child);
			}
		}
		ArrayList<MyDNA> population = oldPopulation;
		population.addAll(newPopulation);

		while (this.getPopulationSize() != finalPopulation.size()){
			finalPopulation.add(getBestIndividual(population));
			population.remove(getBestIndividual(population));
		}
		// YOUR CODE GOES ABOVE HERE
